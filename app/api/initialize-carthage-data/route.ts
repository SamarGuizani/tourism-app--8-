...
export async function GET() {
  try {
    ...
    if (error) {
      console.error("Error initializing Carthage data:", error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json({
      message: "Carthage data initialized successfully",
    })
  } catch (error) {
    console.error("Error in /api/initialize-carthage-data:", error)
    return NextResponse.json(
      {
        error: "Failed to initialize Carthage data",
      },
      { status: 500 },
    )
  }
